...
import { getAllCompliancePolicies } from '../../services/GetRequests'
import { createCompliancePoliciesWithFile } from '../../services/Inserts'
import { updateCompliancePolicies, updateCompliancePoliciesWithFile, deleteCompliancePolicies } from '../../services/UpdRequests'
...
export default function CompliancePoliciesComponent () {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      setError('')
      const response = await getAllCompliancePolicies({ page, size, search, statusId })
  ...
  const handleShowModal = (item = null) => {
    if (item) {
      ...
      setFormData({
        ...
        dateDeMiseAJourDeLaPolice: toInputDate(item.dateDeMiseAJourDeLaPolice),
    ...
    try {
      ...
      if (!editingItem && selectedFile) {
        ...
        const complianceData = {
          ...
          dateDeMiseAJourDeLaPolice: formData.dateDeMiseAJourDeLaPolice ? `${formData.dateDeMiseAJourDeLaPolice}T00:00:00` : null,
          doneBy: { id: Number(CurrentUserId) },
          status: formData.statusId ? { id: parseInt(formData.statusId) } : null
        }

        formDataToSend.append('compliancePolicies', new Blob([JSON.stringify(complianceData)], { type: 'application/json' }))
        await createCompliancePoliciesWithFile(formDataToSend)
      } else if (editingItem) {
        // UPDATE
        const dataToSubmit = {
          reference: formData.reference,
        ...
        if (selectedFile) {
          const formDataToSend = new FormData()
          formDataToSend.append('file', selectedFile)
          formDataToSend.append('compliancePolicies', new Blob([JSON.stringify(dataToSubmit)], { type: 'application/json' }))
          await updateCompliancePoliciesWithFile(editingItem.id, formDataToSend)
        } else {
          await updateCompliancePolicies(editingItem.id, dataToSubmit)
        }
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setError('')
      await deleteCompliancePolicies(itemToDelete.id)
      loadData()
  ...
  return (
    <div className='compliance-policies-component'>
      <style jsx>{`
        .action-buttons .btn {
          font-size: 0.8rem;
          ...
          <Card>
            <Card.Header>
              <Row className='align-items-center mb-3'>
                <Col xs={12} md={6} lg={3}>
                  <HeaderTitle>{getText('document.categoryValues.compliancePolicies', language)}</HeaderTitle>
                </Col>
                  ...
                  <Table striped bordered hover>
                    <thead>
                      <tr>
                        ...
                        <th>{getText('document.fields.dateDeMiseAJourDeLaPolice', language)}</th>
                      ...
                      {data.length === 0 ? (
                        ...
                        data.map((item) => (
                          <tr key={item.id}>
                            ...
                            <td>{item.dateDeMiseAJourDeLaPolice ? new Date(item.dateDeMiseAJourDeLaPolice).toLocaleDateString(language) : '-'}</td>
                            <td>
      ...
      <Modal show={showModal} onHide={handleCloseModal} centered size='lg'>
        <Modal.Header closeButton>
          <Modal.Title>
            {editingItem
              ? `${getText('common.edit', language)} ${getText('document.categoryValues.compliancePolicies', language)}`
              : `${getText('common.add', language)} ${getText('document.categoryValues.compliancePolicies', language)}`}
          </Modal.Title>
        </Modal.Header>
        <Form onSubmit={handleSubmit}>
          <Modal.Body>
            ...
            <Row>
              ...
              <Col md={6}>
                <Form.Group className='mb-3'>
                  <Form.Label>{getText('document.fields.dateDeMiseAJourDeLaPolice', language)}</Form.Label>
                  <Form.Control
                    type='date'
                    name='dateDeMiseAJourDeLaPolice'
                    value={formData.dateDeMiseAJourDeLaPolice}
                    onChange={handleChange}
                  />
                </Form.Group>
              </Col>
            ...
            <Row className='mb-3'>
              <Col md={6}>
                ...
                <ListGroup variant='flush'>
                  ...
                  <ListGroup.Item>
                    <strong>{getText('document.fields.dateDeMiseAJourDeLaPolice', language)}:</strong>{' '}
                    {selectedDocument.dateDeMiseAJourDeLaPolice ? new Date(selectedDocument.dateDeMiseAJourDeLaPolice).toLocaleDateString(language) : '-'}
                  </ListGroup.Item>
                  <ListGroup.Item>
