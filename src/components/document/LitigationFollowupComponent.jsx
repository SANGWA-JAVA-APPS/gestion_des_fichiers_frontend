...
import { getAllLitigationFollowup } from "../../services/GetRequests";
import { createLitigationFollowup, createLitigationFollowupWithFile } from "../../services/Inserts";
import {
  updateLitigationFollowup,
  deleteLitigationFollowup,
  updateLitigationFollowupWithFile,
...
const LitigationFollowupComponent = () => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      setError("");
      const response = await getAllLitigationFollowup({
        page,
        size,
        search,
        statusId,
    ...
    if (item) {
      ...
      setFormData({
        ...
        expectedCompletion: item.expectedCompletion
          ? item.expectedCompletion.split("T")[0]
          : "",
    ...
    try {
      ...
      const dataToSubmit = {
        ...
        expectedCompletion: formData.expectedCompletion
          ? new Date(formData.expectedCompletion).toISOString()
          : null,
        riskValue: formData.riskValue ? parseFloat(formData.riskValue) : null,
      ...
      if (!editingItem && selectedFile) {
        ...
        await createLitigationFollowupWithFile(formDataToSend);
      } else if (editingItem) {
        if (selectedFile) {
          const formDataToSend = new FormData();
          formDataToSend.append("file", selectedFile);
          ...
          await updateLitigationFollowupWithFile(editingItem.id, formDataToSend);
        } else {
          await updateLitigationFollowup(editingItem.id, dataToSubmit);
        }
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setError("");
      await deleteLitigationFollowup(itemToDelete.id);
      loadData();
  ...
  return (
    <div className="litigation-followup-component">
      <style jsx>{`
        .action-buttons .btn {
          font-size: 0.8rem;
      ...
      <Row className="mb-4">
        <Col>
          <Card>
            <Card.Header>
              <PageHeaderCard
                title={language === "fr" ? "Suivi de Litige" : "Litigation Follow Up"}
                language={language}
                  ...
                  <Table striped bordered hover>
                    <thead>
                      <tr>
                        <th>ID</th>
                        <th>
                          {language === "fr" ? "Préoccupation" : "Concern"}
                        </th>
                        <th>
                          {language === "fr"
                            ? "Date de Création"
                        ...
                        <th>
                          {language === "fr" ? "Valeur du Risque" : "Risk Value"}
                        </th>
                  ...
                  {data.length === 0 ? (
                    ...
                    data.map((item) => (
                      <DocumentCard
                        ...
                        getDisplayName={(it) =>
                          it.concern || "Litigation #" + it.id
                        }
                        getDescription={(it) =>
                          it.riskValue
                            ? `${language === "fr" ? "Risque" : "Risk"}: ${it.riskValue}`
                            : ""
                        }
                      />
                    ))
      ...
      <Modal show={showModal} onHide={handleCloseModal} centered size="lg">
        <Modal.Header closeButton>
          <Modal.Title>
            {editingItem
              ? `${getText("common.edit", language)} ${language === "fr" ? "Suivi de Litige" : "Litigation Follow Up"}`
              : `${getText("common.add", language)} ${language === "fr" ? "Suivi de Litige" : "Litigation Follow Up"}`}
          </Modal.Title>
        </Modal.Header>
          ...
          <Modal.Body>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                placeholder={
                  language === "fr"
                    ? "Décrivez le sujet du litige..."
                    : "Describe the litigation concern..."
                }
              />
            </Form.Group>
      ...
      <DocumentDetailsView
        ...
        title={
          selectedDocument?.concern || "Litigation Details"
        }
              ...
              <Col md={6}>
                ...
                <ListGroup variant="flush">
                  ...
                  <ListGroup.Item>
                    ...
                    {selectedDocument.expectedCompletion
                      ? new Date(
                          selectedDocument.expectedCompletion,
                        ).toLocaleDateString(language)
                      : "-"}
...
export default LitigationFollowupComponent;
