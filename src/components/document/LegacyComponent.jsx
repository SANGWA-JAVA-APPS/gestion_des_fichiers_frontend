...
import { getAllLegacy } from '../../services/GetRequests'
import { createLegacyWithFile } from '../../services/Inserts'
import { updateLegacy, updateLegacyWithFile, deleteLegacy } from '../../services/UpdRequests'
...
export default function LegacyComponent () {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      setError('')
      const response = await getAllLegacy({ page, size, search, statusId })
  ...
  const handleShowModal = (item = null) => {
    if (item) {
      ...
      setFormData({
        ...
        dateDebut: toInputDate(item.dateDebut),
        dateFin: toInputDate(item.dateFin),
    ...
    try {
      ...
      if (!editingItem && selectedFile) {
        ...
        const legacyData = {
          ...
          statuts: formData.statuts || null,
          dateDebut: formData.dateDebut ? `${formData.dateDebut}T00:00:00` : null,
          dateFin: formData.dateFin ? `${formData.dateFin}T00:00:00` : null,
          doneBy: { id: Number(CurrentUserId) },
          status: formData.statusId ? { id: parseInt(formData.statusId) } : null
        }

        formDataToSend.append('legacy', new Blob([JSON.stringify(legacyData)], { type: 'application/json' }))
        await createLegacyWithFile(formDataToSend)
      } else if (editingItem) {
        // UPDATE
        const dataToSubmit = {
          description: formData.description,
        ...
        if (selectedFile) {
          const formDataToSend = new FormData()
          formDataToSend.append('file', selectedFile)
          formDataToSend.append('legacy', new Blob([JSON.stringify(dataToSubmit)], { type: 'application/json' }))
          await updateLegacyWithFile(editingItem.id, formDataToSend)
        } else {
          await updateLegacy(editingItem.id, dataToSubmit)
        }
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setError('')
      await deleteLegacy(itemToDelete.id)
      loadData()
    ...
    <div className='legacy-component'>
          ...
          <Card>
            <Card.Header>
              <Row className='align-items-center mb-3'>
                <Col xs={12} md={6} lg={3}>
                  <HeaderTitle>{getText('document.categoryValues.legacy', language)}</HeaderTitle>
                </Col>
                  ...
                  <Table striped bordered hover>
                    <thead>
                      <tr>
                        ...
                        <th>{getText('document.fields.statuts', language)}</th>
                        <th>{getText('document.fields.dateDebutConcession', language)}</th>
                        <th>{getText('document.fields.dateFinConcession', language)}</th>
                      ...
                      {data.length === 0 ? (
                        ...
                        data.map((item) => (
                          <tr key={item.id}>
                            ...
                            <td>{item.coordonneesGps || '-'}</td>
                            <td>{item.statuts || '-'}</td>
                            <td>{item.dateDebut ? new Date(item.dateDebut).toLocaleDateString(language) : '-'}</td>
                            <td>{item.dateFin ? new Date(item.dateFin).toLocaleDateString(language) : '-'}</td>
                            <td>
      ...
      <Modal show={showModal} onHide={handleCloseModal} centered size='lg'>
        <Modal.Header closeButton>
          <Modal.Title>
            {editingItem
              ? `${getText('common.edit', language)} ${getText('document.categoryValues.legacy', language)}`
              : `${getText('common.add', language)} ${getText('document.categoryValues.legacy', language)}`}
          </Modal.Title>
        </Modal.Header>
        <Form onSubmit={handleSubmit}>
          <Modal.Body>
            ...
            <Row>
              ...
              <Col md={6}>
                <Form.Group className='mb-3'>
                  <Form.Label>{getText('document.fields.statuts', language)}</Form.Label>
                  <Form.Control
                    type='text'
                    name='statuts'
            ...
            <Row>
              <Col md={6}>
                <Form.Group className='mb-3'>
                  <Form.Label>{getText('document.fields.dateDebutConcession', language)}</Form.Label>
                  <Form.Control
                    type='date'
                    name='dateDebut'
              ...
              <Col md={6}>
                <Form.Group className='mb-3'>
                  <Form.Label>{getText('document.fields.dateFinConcession', language)}</Form.Label>
                  <Form.Control
                    type='date'
                    name='dateFin'
            ...
            <Row className='mb-3'>
              <Col md={6}>
                ...
                <ListGroup variant='flush'>
                  <ListGroup.Item>
                    <strong>{getText('document.fields.description', language)}:</strong> {selectedDocument.description || '-'}
                  </ListGroup.Item>
                  <ListGroup.Item>
                    <strong>{getText('document.fields.emplacement', language)}:</strong> {selectedDocument.emplacement || '-'}
                  </ListGroup.Item>
                  <ListGroup.Item>
                    <strong>{getText('document.fields.coordonneesGps', language)}:</strong> {selectedDocument.coordonneesGps || '-'}
                  </ListGroup.Item>
                  <ListGroup.Item>
                    <strong>{getText('document.fields.comments', language)}:</strong> {selectedDocument.commentaires || '-'}
                  </ListGroup.Item>
                  <ListGroup.Item>
                    <strong>{getText('document.fields.statuts', language)}:</strong> {selectedDocument.statuts || '-'}
                  </ListGroup.Item>
                  <ListGroup.Item>
                    <strong>{getText('document.fields.dateDebutConcession', language)}:</strong>{' '}
                    {selectedDocument.dateDebut ? new Date(selectedDocument.dateDebut).toLocaleDateString(language) : '-'}
                  </ListGroup.Item>
                  <ListGroup.Item>
                    <strong>{getText('document.fields.dateFinConcession', language)}:</strong>{' '}
                    {selectedDocument.dateFin ? new Date(selectedDocument.dateFin).toLocaleDateString(language) : '-'}
                  </ListGroup.Item>
                  <ListGroup.Item>
